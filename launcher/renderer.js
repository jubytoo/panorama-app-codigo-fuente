...
gridEl.addEventListener('click', async (e) => {
  ...
  try {
    if (act === 'open') {
      ...
      // A2 bajo A3.3 — se decide por `aplicado`, NUNCA por truthiness: antes
      // ponía «Backup restaurado» pasara lo que pasara.
      const resR = await window.launcherAPI.restoreBackup(id, null);
      if (!resR || resR.aplicado !== true) {
        setStatus('NO se ha restaurado: ' + ((resR && resR.error) || 'error desconocido'));
        await psAlert(((resR && resR.error) || 'No se ha podido restaurar el backup.') +
          (resR && resR.reintentable ? '\n\nPuedes volver a intentarlo.' : ''),
          { title: 'No se ha restaurado', danger: true });
        await refresh();
        return;
      }
      if (resR.verificado === false) {
        // Forma 3: la restauración SÍ ocurrió. No se reintenta.
        setStatus('Restaurado, pero hay que reiniciar Panorama.');
        await psAlert(resR.aviso || 'El backup SÍ se ha restaurado, pero no se ha podido verificar. Cierra Panorama del Servicio y vuelve a abrirlo. NO repitas la operación.',
