...
| **P28** — confinamiento de rutas de particiones (path traversal en purgas) | **CERRADO** (19 sept 2026) | Medido: un journal con `particion:persist:..\\victima` (y `.`, `` vacío, `..`, `/`) hacía que el CASO B ejecutara `rmSync` fuera de `Partitions/`: carpeta hermana, `Partitions/` entero y —con `..`— `userData` (se llevó `backups/` y el journal). Contrato explícito en tres niveles (proyecto `persist:proj-<12-14 díg>-<1-6>`; nombre seguro para `borrar-proyecto`; ninguno para `borrar-prep`/`purgar-backups`), UNA tabla `motivoParticionNoValida` aplicada por el lector de journals (⇒ no demostrable, fail-closed, log explícito), el escritor (`ejecutarBorrado` valida antes de escribir), el journal de restauración y la rama genérica; ruta física SOLO por `rutaParticionSeguraParaBorrado` (nombre de proyecto + base `sessionData` + hijo directo por `path.relative`, no `startsWith`). `partition_name` compartido: la BD lo impide (`UNIQUE`, medido en la BD viva); el riesgo residual es la reaparición → P29. Baseline BD viva reanclado a `451EA2C1…`. Batería **95/0**, reversiones **34/0** (11 familias); P27 84/0 + 25/0, P26 151/0 + 28/0, P24 62/0 + 22/0, Bloque 5 80/0 + 58/0, C1 165/0, P18 140/0. Ver §P28 en `pendientes-abiertos.md` |
| **P29** — fronteras restantes de `partition_name` | **ABIERTO — provisional** (19 sept 2026, separado de P28) | Semántica, no contrato de nombre: (a) `borrar-proyecto` + fila reaparecida con la `partition_name` de un borrado ya confirmado (CASO B + `clearStorageData()`; `UNIQUE` no lo impide porque el nombre queda libre al borrar); (b) `row.partition_name` entra sin contrato en ventanas de proyecto, `seedNewProjectStorage`, `backup:restore` (`clearFirst:true`) y `runInPartition`; (c) el inventario de residuos lista `<userData>/Partitions` aunque cuelgan de `sessionData` (P16). Sin implementar. Ver §P29 en `pendientes-abiertos.md` |
...
| | valor |
|---|---|
| **Hash actual (baseline vigente)** | `451EA2C18A2DFF558822B4AC304E50AFB128F7E5F3C6CCB0516DA204D1DF1C95` |
| bytes | 77 824 |
| `LastWriteTime` | 2026-09-19 15:24:04 |
| baseline capturada | 2026-09-19 (ronda P28) |
| **Hash anterior** | `C7F6FC2F640ECE8CFC231B29AAEAF64A18CFEB67E00E61776FF66BE804D9C27E` (baseline 2026-09-18 14:23). **Fue válido hasta la sesión real del 19 sept** (arranque de la v2.0.55 a las 13:23:45Z, backups `startup` y `closing` del proyecto 5; `panorama.sqlite3` escrito a las 15:24:04 local). **El cambio NO lo causaron P26 ni P27 ni ninguna batería:** la última tirada de Electron terminó a las 14:01 y cada batería verificó inicio = fin. No se tocó el contenido real; se reancló el baseline. Ver `comun/baseline-bd-viva.json` |
| **Hash anterior aún** | `C26323D15535949653AC035E13EBBBD084997CCC2AE54AE2119685B8A885AAB0` (capturado 2026-09-17 14:32; cambió por sesiones reales del 18 sept, 10:41–10:45 y 12:22–12:24 local). Antes: `D5C3FF53D09925B6C8258F5EFFF84DA44F949EBCE609A1276887BD30D21F46C8` (2026-09-15 13:24; sesión real del 17 sept) y `52394C7A…` (2026-09-15 00:00). **`C26323D1…`, `C7F6FC2F…` y `451EA2C1…` no tienen el mismo contenido** |
