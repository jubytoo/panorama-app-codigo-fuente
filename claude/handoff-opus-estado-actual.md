...
| **ARN-1** — `nucleo-a33` da 394 / 395 | **PENDIENTE DE ARNÉS — antes del E2E / release final** | Rama «LOCAL, peor caso» que depende de que `utimesSync` devuelva el mtime exacto. No invalida la regresión. **No es producto.** Ver `pendientes-abiertos.md` «Pendientes de ARNÉS» |
...
| **ARN-4** — `RA-1 «el LevelDB CRECE al llamar al flush»` (A2) | **PENDIENTE DE ARNÉS** (18 sept 2026) | Depende de la temporización: 1 fallo en 7 tiradas de `ra1`; la propiedad de fondo pasó en todas. **No es producto** |
| **ARN-5** — la reversión C de P9 (Node) crea `datos\relativa` en el directorio de trabajo | **EFECTO DE ARNÉS REGISTRADO** (18 sept 2026) | Carpeta vacía en la raíz del proyecto, fuera de `build.files` e ignorada por git. No se ha borrado |
| **P23** — la protección de apagado acepta referencias a otra instalación | **ABIERTO — MEDIO** (18 sept 2026) | `syncDriveSyncGuardWithLocation()` no compara la ruta de `Run`/tarea con la instalación en uso. Va con **A3.3 Bloque 8** / ciclo de vida de Drive. Hoy `Run` y la tarea apuntan a la instalación real |
