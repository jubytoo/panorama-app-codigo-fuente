...
| **F2** | **ABIERTO — DIAGNOSTICADO** *(17 sept 2026)* | Ninguna ventana declara CSP. **Confirmado por las dos vías** (0 `<meta>` en 10 HTML, 0 cabeceras en `main.js`). **Medido:** una CSP con `unsafe-inline` (inevitable: 7 ventanas con `<script>` en línea, 9 con estilos en línea) **sí es compatible** con el producto actual, y **NO hace falta `unsafe-eval`** — demostrado en Electron real. Sin implementar. Ver `pendientes-abiertos.md` §F2/F3 |
