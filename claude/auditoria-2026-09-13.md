...
| **P18** — procedencia de la copia de `app.asar` que restaura el rescate | **FASE 1 CERRADA** (18 sept 2026) · **FASE 2 ABIERTA / D4** | El rescate automático solo restaura la predecesora de una operación **VERIFICADA** de este equipo (copia local + registro local + identidad de A3.3); las `app.asar.bak-*` heredadas nunca deciden. «Aplicar parche» ya no rota las copias heredadas en un intento que falla antes de lanzar el ayudante. Solo `main.js` (`486B803F…`). Evidencia: Node **3707/0**, 11 comprobadores de reversiones, barrido Electron oficial de **22 arneses** en sus cifras. Un primer «cerrado» (`9ada9a9`) era un falso verde y se retiró. **Fase 2 / D4:** rescate externo, `.bat`, packaging y purga heredada solo tras VERIFICADA. Detalle en `pendientes-abiertos.md` §P18 |
...
| **F2** | **CERRADO** *(17 sept 2026)* | Las **10** ventanas tienen CSP **efectiva** (leída de Chromium, no solo declarada), entregada por **cabecera** desde un único punto de `main.js` (`session-created` → `onHeadersReceived`), con **cuatro perfiles mínimos** (`cerrado`, `sinScriptEnLinea`, `interfaz`, `lectorDeActas`). Sin `unsafe-eval`; `connect-src`, `frame-src`, `object-src`, `form-action` y `base-uri` a `'none'`; `data:` solo en `img-src`. El lanzador y la splash **sin** `unsafe-inline` en scripts. Preparación arranca el Worker de pdf.js por `blob:` (un Worker `file:` no hereda la CSP, medido). **Límite dicho claro:** donde queda `unsafe-inline`, un `<script>` en línea inyectado sigue ejecutando; el valor de F2 es cortar la salida y los incrustados. Batería **exigente**, Electron real en las 10 ventanas, laboratorio del motor y reversiones. Ver `pendientes-abiertos.md` §F2/F3 |
| **F3** | **CERRADO** *(17 sept 2026)* | Era **[LEÍDO]**; se reprodujo en Electron real y se corrigió. Un helper único en `main.js` (`aplicarPoliticaDeNavegacion`) aplicado a las **10** ventanas: la ventana hija se **deniega siempre**, `http(s)` validado con `new URL` sale a `shell.openExternal`, y `will-navigate` bloquea lo inesperado preservando recarga, ancla y `blob:`. **Ya no aparece la «ventana negra».** Ver `pendientes-abiertos.md` §F2/F3 |
