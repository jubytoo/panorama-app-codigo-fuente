# ---------------------------------------------------------------------------
# E2 - arranque ELECTRON REAL: lanzador y dashboard del mismo proyecto tienen
# que decir lo mismo. En sandbox artificial.
...
Write-Output "service-status.js: $($prodAntes['vendor\service-status.js'].Substring(0,16))"
Write-Output ""

$S = Join-Path $env:TEMP '_a33-e2-real'
if ($S -notlike '*_a33-e2-real*') { throw "sandbox inesperado: $S" }
...
if ($env:E2_MAIN) {
  if (-not (Test-Path $env:E2_MAIN)) { throw "E2_MAIN no existe: $($env:E2_MAIN)" }
  Copy-Item -LiteralPath $env:E2_MAIN -Destination $mainTmp -Force
  $mainAlt = $mainTmp
  Write-Output "MAIN ALTERNATIVO: $($env:E2_MAIN)"
}
...
Write-Output "==================== LANZADOR + DASHBOARD REALES ===================="
...
Write-Output ""
Write-Output "======================================================================"
Write-Output "  E2 ELECTRON REAL: $totOK OK / $totFALLO FALLOS"
Write-Output "======================================================================"
