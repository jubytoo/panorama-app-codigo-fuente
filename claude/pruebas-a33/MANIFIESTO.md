...
TODOS LOS test-*.js, TRAS P18 F1: 3707 OK / 0 FALLOS   (18 sept 2026, cierre de P18 Fase 1; 3750 con comun/)   ← VIGENTE
```

> **Cierre de P18 Fase 1 (18 sept 2026), contra `main.js` = `486B803F…`.** 27
> baterías del criterio, una por proceso, con el node del sistema. **Cuadre
> exacto** con la de arriba:
>
...
> | `p22` | 74 | 75 | +1 |
>
> 3613 + 94 = **3707**. `nucleo-a33` dio **394** (ARN-1). `c1` y `e1` siguen en
> 165 y 47: cambió *cuál* es una aserción, no cuántas.
>
> Arneses ajustados, con su nota en el propio archivo:
>
> - `e1` `E1-M4`: el ancla de hash (C).
> - `c1` `C1-S17`: invertida (B). Era descriptiva de la fuga de `patch-pending`,
>   que el orden C cierra; contra el `main.js` de `9ada9a9` cae, como control.
> - `p9` y `p22`: la cobertura recuperada (P9-14, `Z2a`/`Z2b`).
> - `comun/baseline-bd-viva.json`: reanclada por sesiones reales del usuario (D).
>
> Reversiones: los **11** comprobadores vigentes en verde (B3 7/7, B4 2/2, B5
> 1/1, C1 7/7, F1 19, F2 31, F3 5, P17 6, P9 29, P22 37, P18 45). La reversión G
> de P9 no se generaba desde P22 (`2952711`): ancla reparada.
...
>
> **Corrección (18 sept 2026, cierre de P18 Fase 1): esa última afirmación es
> FALSA.** Los seis leen el `main.js` del proyecto (`PROJ` es la raíz): extraen
> el bloque de A1, A2, B2 o `setMeta` de su texto. Lo que sí sigue siendo cierto
> es que están **fuera del criterio** por ser material anterior a A3.3, roto desde
> entonces. Tampoco se ejecutan en la regresión: son anteriores al guardián de
> rutas actual. Siguen fuera, **dicho en voz alta**, no omitidos en silencio.
...
> **P18 Fase 1 — regresión completa (18 sept 2026).** Contra `main.js` =
> `486B803F…`. No hubo ninguna regresión contra `3A0D7217…` (commit `9ada9a9`),
> que traía la colisión SHA que hizo verde la batería en falso. Detalle y cuadre
> en el resultado «TRAS P18 FASE 1» de arriba.

Contra `main.js` = `486B803FDEF772BECC542125B42675FC270137827F6CDDFFCFEC3A8C7F784491`
(668 791 B, tras P18 Fase 1; antes `3A0D7217…` en `9ada9a9`, con la colisión y sin regresión; `19AAEB1F…`, colisión corregida;
`C4C00809…` tras P22 y el diagnóstico de P18, 639 056 B; `2D05E00B…` tras P9, 610 975 B; `E7D596A8…` tras F2, 601 293 B;
...
> **Tras P18 Fase 1 (18 sept 2026) se reejecutó la tabla ENTERA**: los **22**
> arneses de arriba, cada uno en su propio proceso PowerShell, contra
> `main.js` = `486B803F…`. **Todas las cifras coinciden con las de la tabla.**
> La BD viva quedó idéntica antes y después de cada arnés (`C7F6FC2F…`).
> Ningún arnés arranca el asar empaquetado (ARN-3): todos los envoltorios
> `real-run/` fijan `appData` y `userData` con `setPath` antes de cargar
> `main.js`. P18 no tiene arnés Electron propio, a propósito. Dos incidencias
> en la primera pasada, ninguna de producto:
>
> - `a2/electron-real.ps1` dio **65 / 1** una vez: `RA-1 «el LevelDB CRECE al
>   llamar al flush»` depende de la temporización. La propiedad de fondo («la
>   marca está en disco al confirmar») pasó; `ra1` repetido 5/5 en verde y el
>   arnés completo **66 / 0**. Registrado como **ARN-4**.
> - `p9/electron-p9-revertido.ps1` dio **13 / 1**: la aserción EREV C esperaba
>   PS-1007 y, desde P18, el rescate sin copia verificable avisa con **PS-1025**.
>   Ajustada (se sigue exigiendo el `mkdir` relativo) → **14 / 0**.
>
> No forman parte del barrido, y siguen fuera:
>
...
**Última verificación (18 sept 2026, cierre de P18 Fase 1): 263 listados, 0
faltan, 0 difieren, 0 `.sqlite3`, exit 0.** P18 añade `p18/nombres-modulo.js`,
`p18/revertir-p18.js` y `p18/comprobar-reversiones-p18.js`. Las carpetas
`*/revertidos/` siguen sin listarse: las regeneran los `revertir-*.js`, y
ahora incluyen J y K de P18, M1 y M2 de P22 y la G de P9.

*(Tras el DIAGNÓSTICO de P18:)* **260
