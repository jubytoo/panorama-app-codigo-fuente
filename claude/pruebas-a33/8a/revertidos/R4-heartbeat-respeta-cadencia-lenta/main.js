...
// P23 (18 sept 2026) -- normaliza una ruta de Windows para comparar por
// CONTENIDO, no por formato: separador uniforme y sin distinguir
// mayúsculas/minúsculas (NTFS no las distingue). Mismo criterio que ya usa
// claveUbicacion() en P9/P22 para el mismo tipo de comparación. No hace
// falta hash ni procedencia: el riesgo demostrado es de REFERENCIA
// OBSOLETA (otra carpeta de instalación), no de contenido manipulado en la
// ruta correcta.
function normalizaRutaDriveSyncGuard(p) {
  return path.resolve(String(p)).replace(/\//g, '\\').toLowerCase();
}

function desescaparXmlDriveSyncGuard(s) {
  return String(s)
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
    .replace(/&quot;/g, '"')
...
// sucesos de escala de minutos u horas, no de segundos); el escenario que
// P23 corrige de verdad (otra instalación con guardián vivo) ya está fijado
// en el instante de ESTE arranque. Por eso no hace falta pagar el coste de
// dos procesos externos síncronos en cada watchdog de 45s: se cachea con un
// timestamp simple (sin temporizador nuevo) y se reinspecciona como mucho
// cada 10 minutos -- sigue autorreparándose sola en una sesión larga, solo
// que no en cada tick. El heartbeat y la detección de guardián muerto NO
// pasan por esta caché: siguen mirándose en cada tick, sin cambios.
const DRIVE_SYNC_GUARD_PERSISTENCE_RECHECK_MS = 10 * 60 * 1000;
let ultimaInspeccionPersistenciaMs = 0; // 0 a propósito: la primera vez siempre toca inspeccionar

// P23 -- lee el valor real de HKCU\...\Run\PanoramaDriveSyncGuard y compara
// la ruta del .ps1 que contiene contra la de ESTA instalación. Estados
// EXPLÍCITOS a propósito, nunca un booleano: un fallo de INSPECCIÓN no es
// "está bien" ni "está mal", es otra cosa.
//
// Ajuste final (18 sept 2026): YA NO se distingue "ausente" del resto de
// fallos. Comprobado en vivo, sin depender de texto localizado: tanto
// `reg.exe query` de un valor/clave inexistente COMO de una clave con
// sintaxis inválida devuelven el MISMO código de salida (1) -- no hay
// ninguna señal robusta e independiente del idioma para separar "no existe"
// de "la consulta falló por otro motivo" (ni el código de salida, ni la
// forma de stdout/stderr, que en ambos casos queda vacío). Construir un
// parser de los mensajes en español/inglés sería justo lo que se pidió NO
// hacer. Con "ausente" e "incorrecta" llevando siempre a la MISMA acción,
// la clasificación más segura ante cualquier fallo de lectura (código de
// salida, timeout o error de spawn) es "no-verificable": repara, no lanza
// una copia de más solo por una lectura que no se pudo completar.
function leerRunDriveSyncGuard() {
  let salida;
  try {
    salida = execFileSync(
      'reg.exe',
  ...
  if (!m) return { estado: 'no-verificable', motivo: 'formato de reg.exe inesperado' };
  const valor = m[1].trim();
  const rutaM = /-File\s+"([^"]*)"/i.exec(valor);
  if (!rutaM) return { estado: 'no-verificable', motivo: 'no se pudo extraer la ruta del comando registrado' };
  const estado = normalizaRutaDriveSyncGuard(rutaM[1]) === normalizaRutaDriveSyncGuard(driveSyncGuardScriptPath()) ? 'correcta' : 'incorrecta';
  return { estado, valor, ruta: rutaM[1] };
}

// P23 -- lee la definición real de la tarea PanoramaDriveSyncGuardLaunch y
// compara AMBAS rutas (LaunchHidden.vbs y DriveSyncGuard.ps1) contra las de
// ESTA instalación. Mismo criterio de estados que leerRunDriveSyncGuard()
// (ver su comentario: ya no se distingue "ausente" del resto de fallos).
function leerTareaDriveSyncGuard() {
  let salida;
  try {
  ...
  if (!comandoM || desescaparXmlDriveSyncGuard(comandoM[1]) !== 'wscript.exe') {
    return { estado: 'no-verificable', motivo: 'la tarea no tiene la forma esperada (Command)' };
  }
  const argsM = /<Arguments>([^<]*)<\/Arguments>/.exec(salida);
  if (!argsM) return { estado: 'no-verificable', motivo: 'la tarea no tiene la forma esperada (Arguments)' };
  const rutasM = /^\/\/B\s+"([^"]*)"\s+"([^"]*)"$/.exec(desescaparXmlDriveSyncGuard(argsM[1]));
  if (!rutasM) return { estado: 'no-verificable', motivo: 'no se pudieron extraer las rutas de los argumentos' };
  const vbsOk = normalizaRutaDriveSyncGuard(rutasM[1]) === normalizaRutaDriveSyncGuard(driveSyncGuardVbsLauncherPath());
  const scriptOk = normalizaRutaDriveSyncGuard(rutasM[2]) === normalizaRutaDriveSyncGuard(driveSyncGuardScriptPath());
  return { estado: vbsOk && scriptOk ? 'correcta' : 'incorrecta', vbs: rutasM[1], script: rutasM[2] };
}

// P23 -- combina las dos lecturas. "no-verificable" gana sobre
// "incorrecta": ante duda sobre si de verdad está mal, se prefiere la
// clasificación más conservadora (ver la política en
// syncDriveSyncGuardWithLocation: "no-verificable" repara pero NO lanza
// una copia adicional; "incorrecta" repara Y lanza).
function estadoPersistenciaDriveSyncGuard() {
...
// P23 -- separada de enableDriveSyncGuardSilently(): escribe enabled.flag,
// Run y la tarea apuntando a ESTA instalación, pero NO la ejecuta. Permite
// reparar la persistencia con un guardián YA VIVO (de otra instalación)
// sin arrancar una copia nueva salvo que se pida explícitamente (ver
// `alTerminar`) — y sin tocar el proceso que ya está vivo. `alTerminar(ok)`
// se llama una única vez, cuando termina el intento de crear la tarea (con
// éxito o no) — así quien la invoque puede esperar a que la tarea exista
// de verdad antes de pedir `schtasks /run` (ver lanzarDriveSyncGuardActual),
// sin la carrera de pedirlo antes de que `/create` haya terminado.
function repararPersistenciaDriveSyncGuard(reason, alTerminar) {
  const listo = (ok) => { if (typeof alTerminar === 'function') alTerminar(ok); };
  if (process.platform !== 'win32') { listo(false); return; }
...
// P23 -- separada de enableDriveSyncGuardSilently(): dispara YA la tarea
// (debe haberse creado antes, ver repararPersistenciaDriveSyncGuard — si
// no existiera todavía, `schtasks /run` simplemente falla, con su propio
// aviso, igual que antes). Hace ÚNICAMENTE esto, sin tocar enabled.flag,
// Run ni la tarea, y sin intentar terminar ningún proceso ya vivo.
function lanzarDriveSyncGuardActual(reason) {
  ...
  execFile('schtasks.exe', ['/run', '/tn', DRIVE_SYNC_GUARD_TASK_NAME], { windowsHide: true }, (runErr) => {
    if (runErr) {
      ...
      try {
        fs.appendFileSync(diagPath, `[${new Date().toISOString()}] Error al disparar la tarea programada (${reason}): ${String((runErr && runErr.message) || runErr)}\n`, 'utf8');
      } catch (e2) { /* no crítico */ }
    } else {
      try {
        fs.appendFileSync(diagPath, `[${new Date().toISOString()}] Tarea programada disparada correctamente (${reason}) -- esto NO confirma que el script en sí haya arrancado -- eso lo dicen trace-arranque.log y heartbeat.txt).\n`, 'utf8');
      } catch (e2) { /* no crítico */ }
    }
  });
}

// v0.1.54 -- activación normal (sin carpeta compartida antes / al
// desactivarse el proceso). P23 (18 sept 2026): reutiliza las dos piezas de
// arriba, para que el comportamiento existente quede exactamente igual
// (repara la persistencia y, si sale bien, lanza ya mismo).
function enableDriveSyncGuardSilently(reason) {
  repararPersistenciaDriveSyncGuard(reason, (ok) => {
    if (ok) lanzarDriveSyncGuardActual(reason);
  });
...
// Block 8B — misma fórmula que usa el propio lock para identificarse, para
// poder comparar "¿el lock que hay en disco somos nosotros mismos?" sin
// duplicar el criterio en dos sitios que podrían desincronizarse.
function currentMultiPcIdentity() {
  return {
...
// Una identidad solo cuenta como fiable si machine/user son de verdad los
// del equipo (nunca los valores de reserva "desconocido") — así dos equipos
// que por lo que sea no pudieran identificarse nunca coinciden entre sí por
// accidente.
function esIdentidadMultiPcFiable(id) {
  return !!(id
    && typeof id.machine === 'string' && id.machine && id.machine !== 'equipo desconocido'
    && typeof id.user === 'string' && id.user && id.user !== 'usuario desconocido');
}

// Block 8B — LOCK RESIDUAL LOCAL (misma máquina + mismo usuario). Un lock
// fresco que coincide EXACTAMENTE con la identidad de este proceso solo
// puede ser un residuo de una sesión anterior de esta misma instalación que
// no cerró limpiamente (crash, kill, corte de luz): nunca un proceso vivo
// de verdad. app.requestSingleInstanceLock() (ver la cabecera de este
// archivo, A3.1) ya lo habría impedido antes de llegar hasta aquí, porque
// el cerrojo de instancia única se pide con el userData POR DEFECTO de este
// mismo perfil de Windows — validado en sandbox (Block 8B, sin Drive real):
// mismo userData por defecto → el segundo proceso nunca consigue el
// cerrojo; userData distinto (equivalente a otro usuario de Windows) →
// los dos lo consiguen, sin arbitraje entre ellos. Por eso NO basta con
// comparar solo `machine`: un usuario de Windows distinto en este mismo
// equipo tiene su propio dominio de instancia única, y ahí sí podría haber
// un proceso vivo real que este lock nunca vería.
//
// Ante cualquier duda sobre la identidad (campos ausentes, o "desconocido"
// en cualquiera de los dos lados) NO se toma solo: sigue el camino
// conservador de siempre (preguntar).
//
// Esto NO resuelve el caso de dos equipos reales, ni el de este mismo
// equipo con Drive caído mientras otro PC entra de verdad — eso sigue
// abierto (ver pendientes-abiertos.md, «Block 8B — offline/multi-PC»).
function esResiduoLocalPropio(existing) {
  const actual = currentMultiPcIdentity();
  if (!esIdentidadMultiPcFiable(existing) || !esIdentidadMultiPcFiable(actual)) return false;
...
async function writeMultiPcLockNow() {
  try {
    const info = Object.assign({}, currentMultiPcIdentity(), { lastUpdate: new Date().toISOString() });
