'use strict';
// Diff unificado sin git (no está instalado en esta máquina).
const fs = require('fs');
const path = require('path');
const A = 'C:\\Codigo Fuente PS\\panorama-app-codigo-fuente_1\\claude\\db.js.ANTES-BLOQUE1-2026-09-14';
const B = 'C:\\Codigo Fuente PS\\panorama-app-codigo-fuente_1\\db.js';
const OUT = path.join(__dirname, 'db.js.diff');
const CTX = 3;

const a = fs.readFileSync(A, 'utf8').split(/\r?\n/);
const b = fs.readFileSync(B, 'utf8').split(/\r?\n/);

// LCS por programación dinámica con recorte de prefijo/sufijo común.
let ini = 0;
...
// Agrupar en hunks con contexto
const cambios = ops.map((o, idx) => (o[0] !== ' ' ? idx : -1)).filter((x) => x >= 0);
const salida = [];
salida.push('--- db.js (ANTES del bloque 1)');
salida.push('+++ db.js (DESPUÉS del bloque 1)');
let n = 0;
while (n < cambios.length) {
  ...
  const nA = trozo.filter((o) => o[0] !== '+').length;
  const nB = trozo.filter((o) => o[0] !== '-').length;
  const pA = (trozo.find((o) => o[0] !== '+') || [])[2] || 0;
  const pB = (trozo.find((o) => o[0] !== '-') || [])[3] || 0;
  salida.push(`@@ -${pA},${nA} +${pB},${nB} @@`);
  trozo.forEach((o) => salida.push(o[0] + o[1]));
  n = m + 1;
}
fs.writeFileSync(OUT, salida.join('\n'), 'utf8');
const mas = ops.filter((o) => o[0] === '+').length;
const menos = ops.filter((o) => o[0] === '-').length;
console.log('diff escrito en ' + OUT);
console.log('  lineas antes:   ' + a.length);
console.log('  lineas después: ' + b.length);
console.log('  añadidas:  +' + mas);
console.log('  quitadas:  -' + menos);
console.log('  hunks:     ' + salida.filter((l) => l.startsWith('@@')).length);
console.log('');
console.log('--- cabeceras de los hunks ---');
